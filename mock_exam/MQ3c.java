...
class Student  {
    private int marks;
    Map<Grade, Integer> gradedStudents = new HashMap<Grade, Integer>();
    List<Student> students = new ArrayList<Student>();
    static List<Grade> status = new ArrayList<Grade>();
    ...
    public static void main(String[] args) {
        ...
        s.gradedStudents();
        s.calculateGradeStatistics(status);
        System.out.println(s.gradedStudents);
    }
    ...
    public void gradedStudents() {
        ...
        for (Student s : students) {
            if (s.getMarks() >= 80) {
                status.add(Grade.GradeDistinction);
            } else if (s.getMarks() >= 60) {
                status.add(Grade.GradePass);
            } else {
                status.add(Grade.GradeFail);
            }
        }
    ...
    //> grade stats
    public void calculateGradeStatistics(List<Grade> status) {
        int failCount = 0;
        int passCount = 0;
        int distinctionCount = 0;

        for (Grade g : status) {
            if (g == Grade.GradeFail) {
                failCount++;
            } else if (g == Grade.GradePass) {
                passCount++;
            } else {
                distinctionCount++;
            }
        ...
        //? adding to map
        gradedStudents.put(Grade.GradeFail, failCount);
        gradedStudents.put(Grade.GradePass, passCount);
        gradedStudents.put(Grade.GradeDistinction, distinctionCount);
